...
import { 
  ...
  RefreshCw, Users, CheckCircle, AlertCircle, Loader2, 
  Activity, MapPin, Tag, Phone, Calendar, DollarSign 
} from 'lucide-react'
...
const SkeletonRow = () => (
  ...
  const [formData, setFormData] = useState({ client_name: '', contact_number: '', location: '', plan_id: '', installation_date: '', status: 'active' })
  ...
  useEffect(() => {
  const channel = supabase.channel('clients_channel').on('postgres_changes', { event: '*', schema: 'public', table: 'clients' }, () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setPlans(p.data || [])
      setLocations(l.data || [])
    } catch (e) { setToast({ message: 'Failed to load data', type: 'error' }) }
    finally { setIsLoading(false); setLastSync(new Date()) }
  }
  useEffect(() => { fetchData() }, [])

  const calculateDueDate = (d: string) => {
    if (!d) return ''
    const [y, m, day] = d.split('-').map(Number)
    let nm = m + 1, ny = y
    if (nm > 12) { nm = 1; ny = y + 1 }
    const days = new Date(ny, nm, 0).getDate()
    return `${ny}-${String(nm).padStart(2, '0')}-${String(Math.min(day, days)).padStart(2, '0')}`
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!error) setToast({ message: editId ? 'Updated' : 'Added', type: 'success' })
      else setToast({ message: error.message, type: 'error' })
    } catch (e) { setToast({ message: 'Error occurred', type: 'error' }) }
    setShowModal(false)
  ...
  const handleDelete = async (id: any) => {
    if (confirm('Delete this client?')) {
      ...
      if (!error) { setToast({ message: 'Deleted', type: 'success' }); fetchData() }
      else setToast({ message: error.message, type: 'error' })
      setProcessingId(null)
    }
  }
  ...
  const openEdit = (client: any) => {
    setFormData({ client_name: client.client_name, contact_number: client.contact_number, location: client.location, plan_id: client.plan_id, installation_date: client.installation_date || '', status: client.status || 'active' })
    setEditId(client.id)
    setShowModal(true)
  } 
  return (
    <>
      <style>{styles}</style>
      <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-8 overflow-hidden flex flex-col relative">
        <div className="fixed inset-0 bg-gradient-to-br from-cyan-500/5 via-blue-500/5 to-purple-500/5 -z-10" />
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium text-slate-400">{isLive ? 'Live Updates' : 'Connected'}</span>
          </div>
          <div className="text-xs text-slate-500">Last Sync: {lastSync.toLocaleTimeString()}</div>
        </div>

        {/* Title & Actions */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h2 className="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">Clients Management</h2>
            <p className="text-slate-400 mt-1">Manage your client database</p>
          </div>
          <div className="flex gap-2">
            <button onClick={fetchData} className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 px-4 py-2 rounded-lg text-white transition-all border border-slate-700">
              <RefreshCw size={18} className={isLoading ? 'animate-spin' : ''} /> Refresh
            </button>
            {isAdmin && (
              <button onClick={() => { setEditId(null); setFormData({ client_name: '', contact_number: '', location: '', plan_id: '', installation_date: '', status: 'active' }); setShowModal(true) }} className="group flex items-center gap-2 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 px-4 py-2 rounded-lg text-white transition-all hover:shadow-lg hover:shadow-cyan-500/30">
                <Plus size={20} className="group-hover:rotate-90 transition-transform" /> Add Client
              </button>
            )}
          </div>
        </div>
        ...
        {/* Clients Table */}
        <div className="overflow-x-auto bg-slate-900/50 rounded-xl  border-slate-800 transition-all duration-300 backdrop-blur-sm">
